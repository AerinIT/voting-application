...
app.post("/api/topics/:topic/vote", async (req, res) => {
  ...
  diag.info(
    `Processing vote for topic: ${topic} - Name: ${name}, Vote: ${vote}`
  );
...
app.get("/api/topics/:topic/results", async (req, res) => {
  ...
  diag.info(
    `Results fetched for topic: ${topic} - Agree: ${agreeVotes.length}, Not Agree: ${notAgreeVotes.length}`
  );
  res.json({
    topic,
...
// // Basic example endpoint (optional)
// app.get("/", (req, res) => {
