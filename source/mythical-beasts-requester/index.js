...
const { accumulators } = require('./endpoints')();  // Removed nameSet
...
// Global span context for linking.
...
const makeRequest = async (tracingObj, sendMessage, logEntry) => {
    ...
    const endpoints = ['/create', '/update', '/delete', '/info']; // Directly define the endpoints
    const endpoint = endpoints[Math.floor(Math.random() * endpoints.length)];
    const dangerLevel = accumulators[Math.floor(Math.random() * accumulators.length)];
    let headers = {};
    let error = false;

    // Time-shift function for logs
    let timeshift = () => {
        var date = (process.env.TIMESHIFT) ? new Date(Date.now() - (1000 * 60 * 40)) : new Date(Date.now());
        return date.toISOString();
    }

    // Create a new span, link to previous request
    ...
    // Request context and span propagation
        ...
        if (type === 'GET') {
            ...
            try {
                ...
                logEntry({
                    ...
                    job: 'mythical-requester',
                    endpointLabel: 'requester',
                    endpoint,
                ...
                if (Math.random() < 0.1 && names.length > 0) { // 10% chance to delete a name
                    await axios.delete(`http://${serverHostPort}${endpoint}`, { data: { name: names[0].name }, headers });
                ...
                logEntry({
                    ...
                    endpointLabel: 'requester',
                    endpoint,
                    message: `traceID=${traceId} http.method=GET endpoint=${endpoint} status=FAILURE loggedtime=${timeshift()}`,
                });
                error = true;
            }
        } else {
            ...
            try {
                ...
                logEntry({
                    ...
                    endpointLabel: 'requester',
                    endpoint,
                    message: `traceID=${traceId} http.method=POST endpoint=${endpoint} name=${randomName} loggedtime=${timeshift()} status=FAILURE`,
                });
                error = true;
            }
        }
    ...
    // Add spikes to request rate for better graphing
    counter++;
    if (counter >= 3000) {
        counter = 0;
    }
